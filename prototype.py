...
def speak(original_audio,silence_list,final_file_name):
	...
	for i in range(0,len(silence_list)+1):
		...
		final=final_file_name+".wav"
		final_file.export(final,format="wav")
	F=final_file_name+".wav"	
	...
	if X>Y:
		...
		temp=temp+speak[0:X]
		F=final_file_name+".wav"
		temp.export(F,format="wav")
		



def main(name_of_audio,lan,final_name):
	final="OUTPUT_FOLDER"+"/"+final_name
	f=name_of_audio
...
f=#File_name
main(f,#preferred_language,#Prefered_output_file_name)
